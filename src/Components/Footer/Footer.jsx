...
export default memo(function Footer() {
  const { datas: usefulLinks } = useFetch("menus/topbar" , false);
  const { datas: infos } = useFetch("infos/index" , false);
  const [newsLatterValue, setNewsLatterValue] = useState("");
  const ShuffledLinks = useCallback(
    [...usefulLinks].sort(() => 0.5 - Math.random()),
    [usefulLinks]
  );
  const {
    register,
  ...
  const NewsLatterHandler = (data) => {
    console.log(data);
    const ResponseResult = ApiRequest.post("newsletters", {
      email: data.Email,
    }).then((response) => {
      ...
      <div className="container">
        ...
        <div className="flex justify-between flex-wrap gap-y-5 gap-x-4 pb-5 border-b dark:border-b-gray-700">
          ...
          <div className="flex flex-col items-center sm:items-start gap-y-5 sm:flex-grow">
            <h4 className="font-DanaMd text-2xl text-zinc-700 dark:text-white">
              درباره ما
            </h4>
            ...
            <div className="w-full flex-between">
              <div className="flex-center gap-3">
                <p className="flex-center bg-emerald-100 dark:bg-mainSlate text-primary size-12 font-DanaBold text-2xl p-2 rounded-full">
                  {infos.coursesCount}
                </p>
                <span>دوره</span>
              </div>
          ...
          <FooterLinks
            title=" لینک های مفید"
            linkList={ShuffledLinks.slice(0, 5)}
          />
          {/* Social Media */}
            ...
            <div className="flex flex-col items-start gap-y-3">
              <div className="flex items-center gap-x-4">
                ...
                <Link
                  ...
                  className="dir-ltr text-hover font-DanaBold"
                >
                  @sabzlearn_
                </Link>
              </div>
              ...
              <div className="flex items-center gap-x-4">
                ...
                <Link
                  ...
                  className="dir-ltr font-DanaBold"
                >
                  @sabzlearn
                </Link>
              </div>
              <div className="flex flex-col gap-4">
                ...
                <div className="shadow-light dark:shadow-none bg-gray-100 dark:bg-gray-800 dark:border border-gray-700 rounded-2xl overflow-hidden">
                  <form
                    className="h-full"
                    onSubmit={handleSubmit(NewsLatterHandler)}
                  >
                    <div
                      className={`${
                        errors.Email && "border border-rose-500"
                      } h-full flex-between text-slate-500 dark:text-gray-500`}
                    >
                      <input
                        type="text"
                        {...register("Email", {
                      ...
                      <Button
                        btnType="submit"
                        className="h-full px-2 bg-primary text-white"
                ...
                <span className="block text-rose-500 text-sm">
          ...
          <div className="w-full flex justify-center xs:justify-between flex-wrap gap-x-3 gap-y-2 py-5 text-base">
            ...
            <p className="dir-ltr text-center">
