...
function Orders() {
  ...
  const columns = [
        ...
        return (
          <div className="flex flex-col items-center gap-5">
            <p className="opacity-60 line-through">
              {course.row.price !== course.row.course.price
                ? course.row.course.price.toLocaleString()
                : ""}
            </p>
            <p className="text-primary font-DanaBold">
              {course.row.course.price === 0
                ? "رایگان"
                : course.row.price.toLocaleString()}
            </p>
          </div>
        );
      },
    },
    {
      ...
      align: "center",
      renderCell: (course) => {
        return course.row.course.price === course.row.price ? (
          "ندارد"
        ) : (
          <span className="font-DanaBold">
            {(course.row.course.price - course.row.price).toLocaleString()} -
          </span>
        );
      },
    },
      ...
      {isShowLoading ? (
        ...
        <>
          <div className="w-full dark:text-white">
            <h2 className="font-DanaBold my-8 text-2xl">
          ...
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                >
                  {oneCourse.isComplete === 1 ? (
                    <span className="bg-emerald-50 text-primary p-1 rounded-lg font-DanaBold">
                      تکمیل شده
                    </span>
                  ) : (
                    <span className="bg-amber-50 text-amber-600 p-1 rounded-lg">
                      در حال ضبط
                    </span>
                  )}
                </th>
                ...
                <td className="px-6 py-4">{oneCourse.support}</td>
                <td className="px-6 py-4">
